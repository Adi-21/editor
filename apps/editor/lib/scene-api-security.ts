...
const ALLOWED_HEADERS = 'authorization, content-type, if-match, last-event-id, x-beeaver-scene-token'
...
function validateAuth(request: Request): NextResponse | null {
  const token = process.env.BEEAVER_SCENE_API_TOKEN
  if (!token) {
    if (isLoopbackRequest(request)) return null
    return sceneApiJson(request, { error: 'scene_api_token_required' }, { status: 503 })
  }

  const supplied = bearerToken(request) ?? request.headers.get('x-beeaver-scene-token')
...
function rateLimitPerMinute(): number {
  const raw = process.env.BEEAVER_SCENE_API_RATE_LIMIT
...
function configuredOrigins(): Set<string> {
  const raw = process.env.BEEAVER_SCENE_API_ORIGINS
